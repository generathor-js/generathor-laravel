...
class StringHelper {
	...
	public fromSnakeToPluralSnake(text: string): string {
		...
		return parts
			.map((part, index) => {
				return index === parts.length - 1 ? pluralize.plural(part) : pluralize.singular(part);
			})
			.join('_');
	}
	...
	public fromSnakeToPluralLabel(text: string) {
		const parts: Array<string> = text.toLowerCase().split('_');
	...
	public fromSnakeToSingularLabel(text: string) {
		return this.capitalize(text.toLowerCase().split('_').map(pluralize.singular).join(' '));
	}
	...
	public toPluralKebab(text: string) {
		...
		parts[parts.length - 1] = pluralize.plural(parts[parts.length - 1] as string);
