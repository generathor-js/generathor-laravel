...
class Naming {
	public relationAttribute(relation: Relation, table: string) {
		...
		if (relation.type !== 'has-many') {
			return str.fromSnakeToSingularCamel((relation.columns[0] as string).replace(/_id$/, ''));
		}

		//Has many relationship
		const fk = (relation.references[0] as string).replace(/_id$/, '');
		if (table === str.fromSnakeToSingularSnake(fk) || table === str.fromSnakeToPluralSnake(fk)) {
			return str.fromSnakeToPluralCamel(relation.on.table);
		}

		return `${str.fromSnakeToPluralCamel(relation.on.table)}As${str.fromSnakeToSingularPascal(fk)}`;
	}
	...
	public searchContext(table: string) {
		return str.fromSnakeToSingularKebab(table);
	}
	...
	public relationContext(relation: Relation, table: string) {
		...
		if (relation.type !== 'has-many') {
			return str.fromSnakeToSingularKebab((relation.columns[0] as string).replace(/_id$/, ''));
		}
		...
		return `${str.fromSnakeToPluralKebab(relation.on.table)}-as-${str.fromSnakeToSingularKebab(fk)}`;
	}
	...
	public relationLabel(relation: Relation, table: string) {
		...
		if (relation.type !== 'has-many') {
			return str.fromSnakeToSingularLabel((relation.columns[0] as string).replace(/_id$/, ''));
		}
		...
		return `${str.fromSnakeToPluralLabel(relation.on.table)} as ${str.fromSnakeToSingularLabel(fk).toLowerCase()}`;
	}
